...
void exibe_AttributeInfo(attribute_info attributeInfo, uint32_t index, cp_info *constantPool, uint8_t indentation) {
    ...
    if (Utils::compararUtf8String(attributeUtf8, "ConstantValue")) {
        ...
        for (uint16_t i = 0; i < innerClassAttr.number_of_classes; i++) {
            ...
            Utils::exibeTabs(indentation+1);
            printf("%d\t ", i);
            printf("cp_info #%d <%s>\t ", innerClass.inner_class_info_index, getFormattedConstant(constantPool, innerClass.inner_class_info_index));
            printf("cp_info #%d <%s>\t ", innerClass.outer_class_info_index, getFormattedConstant(constantPool, innerClass.outer_class_info_index));
