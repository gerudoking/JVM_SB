...
void Reader::FillAttribute(FILE *file, ClassFile *jvm_class, attribute_info *attribute) {
	...
	if (!strcmp(attributeType, "ConstantValue")) {
		...
		attribute->info.CodeAttribute.exception_table = (struct exception_tab *) malloc(
				exception_table_length * sizeof(struct exception_tab));
		...
		attribute->info.LineNumberTable.line_number_table = (struct line_number_table *) malloc(
				attribute_tamanho * sizeof(struct line_number_table));
